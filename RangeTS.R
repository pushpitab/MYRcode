...
battery_load<-c(50,150,300,600,1200)
fuel_capacity<-c(0,50,100,150,300)
no_rotor <-c(1,4,6,8,12)
...
calc.consequence <- function(choise_grid1,consequenceTable) {
 ...
 if(choise_grid1$fuel_capacity>0){
  ...
  if(consequenceTable$Electrical_Energy1>0){
    ...
    consequenceTable$Obtainable_Energy<- (consequenceTable$Gas_Energy*parameters$Gas_to_elect_eff
    +consequenceTable$Electrical_Energy1*parameters$Electric_to_kinetic_eff)
...
pb <- txtProgressBar(min = 0, max = 67500, style = 3)
...
write.csv(output,file="output_new.csv")
